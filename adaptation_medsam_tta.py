...
def train_sam(
    ...
    for epoch in range(0, num_epochs):
        for iter, data in enumerate(all_dataloader):
            ...
            ################################################
            transformed_images = transform_auto(images_test[0][0:1], basenames[0])  # 输入单通道 [1, 1024, 1024]
            ...
            with torch.no_grad():
                transformed_images = transform_auto(images_test[0][0:1], basenames[0]) 
                transformed_images = transformed_images.unsqueeze(0)  
                _, pred_masks, ious, _ = model(transformed_images, prompts)
                _, pred_masks_2, ious, _ = anchor_model(transformed_images, prompts)
                
                for pred_mask, pred_mask_2, gt_mask, basename, iou in zip(pred_masks, pred_masks_2, gt_masks_test, basenames, ious):
                    # pred_masks = (pred_mask + pred_mask_2)/2.0
...
def load_config(cfg_path: str, args: argparse.Namespace):
    ...
    cfg.out_dir = "output/medsam_tta_SBCT/"+args.dataset
