...
class LearnableBezierTransform(nn.Module):
    ...
    def forward(self, x):
        ...
        self.save_image(x[0], f1[0], f2[0], 'transformed')
    ...
    def bezier_curve(self, control_points, x):
        ...
        Args:
            control_points (torch.Tensor): 控制点，形状为 [4]。
            x (torch.Tensor): 输入图像，形状为 [1, H, W]，值范围 [0,1]。
        ...
        """
        # 控制点y坐标
        P0, P1, P2, P3 = control_points  # 每个是标量
        ...
        # 计算变换后的像素值
        # f(t) = B0 * P0 + B1 * P1 + B2 * P2 + B3 * P3
        # 使用y坐标来映射灰度值
        f_t = B0 * P0 + B1 * P1 + B2 * P2 + B3 * P3  # [1, H, W]
...
if __name__ == "__main__":
    ...
    # 保存图像
    transform.save_image(original, transformed1, transformed2, 'transformed.png')
