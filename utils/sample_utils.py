...
def uniform_sampling33(masks, N=5):
    n_points = []
    for mask in masks:
        if not isinstance(mask, np.ndarray):
            mask = mask.cpu().numpy()
        ...
        # 正样本采样：前景区域的采样
        foreground_indices = np.argwhere(mask == 1)  # [y, x]
        if len(foreground_indices) == 0:
            po_points = []  # 如果没有前景，返回空列表
        else:
            sampled_indices = np.random.choice(len(foreground_indices), N, replace=True)
            po_points = np.flip(foreground_indices[sampled_indices], axis=1)  # [x, y]
        
        # 负样本采样：背景区域的采样
        background_indices = np.argwhere(mask == 0)  # [y, x]
        if len(background_indices) == 0:
            na_points = []  # 如果没有背景，返回空列表
        else:
            sampled_indices = np.random.choice(len(background_indices), N, replace=True)
            na_points = np.flip(background_indices[sampled_indices], axis=1)  # [x, y]

        n_points.append((po_points.tolist(), na_points.tolist()))  # 返回正负样本点
