...
from utils.eval_utils import AverageMeter, calc_iou, validate, get_prompts, validate_med, test_med
...
def train_sam(
    ...
    for epoch in range(1, num_epochs):
        ...
        if epoch % cfg.eval_interval == 0:
            dice = validate_med(fabric, cfg, model, val_dataloader, cfg.name, epoch * num_iters)
            if dice > max_dice:
                state = {"model": model, "optimizer": optimizer}
                fabric.save(os.path.join(cfg.out_dir, "save-ckpt", f"{cfg.dataset}-{cfg.prompt}-last-ckpt.pth"), state)
                max_dice = dice
    ############################ Test the final result
    full_checkpoint = fabric.load(os.path.join(cfg.out_dir, "save-ckpt", f"{cfg.dataset}-{cfg.prompt}-last-ckpt.pth"))
    model.load_state_dict(full_checkpoint["model"])
    test_med(fabric, cfg, model, test_dataloader, name=cfg.name, iters=cfg.num_iters)
...
def main(cfg: Box, ckpt: str = None) -> None:
    ...
    train_data, val_data, test_data = load_datasets(cfg, model.model.image_encoder.img_size)
    optimizer, scheduler = configure_opt(cfg, model.model)

    fabric.print(f"Train Data: {len(train_data) * cfg.batch_size}; Val Data: {len(val_data) * cfg.val_batchsize}; Val Data: {len(test_data) * 1}")
    ...
    validate_med(fabric, cfg, anchor_model, val_data, name=cfg.name, iters=0)

    train_sam(cfg, fabric, model, anchor_model, optimizer, scheduler, train_data, val_data, test_data, num_iters)
